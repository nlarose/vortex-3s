...
#pragma config(Sensor, dgtl5,  ultrason,       sensorSONAR_cm)
...
//	 * Platform Type = Natural Language 2.0
/////////////////////////////////////////////////////////////////////////////////////////
...
task autonomous()
{
	...
	do
	{
		distance = SensorValue[ultrason];
	}
	while(distance > 30);

	setMultipleMotors(-50, baseAvdroit, baseArrDroit, baseAvGauche, baseArrGauche);

	while( distance < 120 )
	{
		lectureDistance = SensorValue[ultrason];
		if (abs(lectureDistance - distance) < 30)
		{
			distance = lectureDistance;
		}
	} // Fin de la boucle de distance..

	// Arrete le robot.
	stopMultipleMotors(baseAvdroit, baseArrDroit, baseAvGauche, baseArrGauche);

	// Baisse la pelle.
	setMultipleMotors(-75,PelleDroite,PelleGauche);
	sleep(500);
	stopMultipleMotors(PelleDroite,PelleGauche);


	// FAire monter l'elevateur.
	setMultipleMotors(75, ElevArrGauche, ElevAvGauche, ElevAvDroit, ElevArrDroit);
	// Attendre que l'elevateur soit monte.
	while(SensorValue[llimithaut] == 1)
	{
	}
	//Arreter l'elevateur.
  ...
  SensorValue[piston1]= 1; //activer les pistons
	...
	sleep(250);	// Attendre 250ms.
	SensorValue[piston2]= 0; //desactiver les pistons
	SensorValue[piston1]= 0;


	// FAire descendre l'elevateur.
	setMultipleMotors(-75, ElevArrGauche, ElevAvGauche, ElevAvDroit, ElevArrDroit);
	// Attendre que l'elevateur soit monte.
	while(SensorValue[limitbas] == 1)
	{
	}
	//Arreter l'elevateur.
	stopMultipleMotors(ElevAvGauche, ElevArrGauche, ElevArrDroit, ElevAvDroit);
